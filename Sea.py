...
def sea(ip, d):
    ...
    time.sleep(2)
    # 星期一~星期五 早上10點到晚上12點
    if datetime.datetime.now().weekday() < 5 and 10 <= datetime.datetime.now().hour < 24 and ip !='emulator-5558':
        for i in range(2):
            # d.swipe(100,400,400,400)
            d.swipe(400,400,100,400)
        if img_tools.click_str_by_server(d,"資源",x_range=(137,425)):
            time.sleep(0.5)
