...
- 以 `time_budget_ms=250` 為硬上限；逾時回傳目前最佳結果。
- 規劃器輸出完整最佳路徑（至捲動點為止，不猜下一個 viewport）。執行語意依後端：
  WS supervised 迴圈每輪只執行第一步、取得新盤面與庫存後重新規劃；
  CNN/ADB 後端整批執行完整路徑，攤平每步截圖+分類成本（與 v1 同等待遇）。
  （2026-07-12 更新：原設計「每輪只執行第一步」僅保留於 WS 路徑；ADB 整批執行
  是有意的重設計，經真實盤面 replay 與離線 A/B 驗證。）
...
2. 本輪與搜尋視野內採集的礦坑格收益。
3. 鎬子成本。
4. 炸彈與鑽頭的相同基礎成本，且**依單次使用實際命中的礦坑格數分級**：
   命中 >=2 礦與 1 礦維持基礎成本；0 礦（純挖掘用途）大幅加價。
   理由：統一固定成本時 beam search 會把道具當便宜挖掘機（一發開 ~10 格
   vs 逐格 1 鏟），道具用量暴增但綜合資源效率 pit/(shovel+3*item) 反輸 v1；
   分級後挖掘交還給鎬子與位能導向，高收益道具使用（多礦覆蓋、破 rock、
   已知畫面外礦）不受影響。
5. 未採集礦被捲出畫面的懲罰。
6. 半挖 cluster 在 session 結束時未完成的風險。
7. 沒有礦時，以低權重獎勵最低成本的向下推進。

路徑開通可以加分，但權重必須低於直接收礦與完成 cluster。不得保留固定的炸彈優先或鑽頭優先 tie-break；效果完全相同時只使用座標等穩定排序，確保決策可重現。具體規則（2026-07-12）：同分時 dig 先於道具（省道具）；bomb/drill 之間以目標座標 parity（`(row+col) % 2`）決定先後——決策可重現、長期用量約 50/50，不偏耗單一道具。
...
**2026-07-12 更新（使用者指示）**：使用者已明確指定所有裝置切換為 `final_v1`
（commit 0ca52d62），「是否切換」不再是門檻問題；上述勝出標準轉為**持續優化
目標**——final_v1 必須在礦坑產出（pits、clusters）**與**綜合資源效率
`pits/(shovel + 3*items)` 上同時優於 v1（首輪重設計已達成產出面 +11%/+2.4%
與鏟效 2 倍，但 pit/eq 0.170/0.177 仍小輸 v1 0.180/0.181，缺口在道具超耗，
由產出分級道具成本修正）。
