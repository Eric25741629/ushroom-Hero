...
| `PageDetector` / `try_detect_main_page_fast` | `utils/page_detector.py` | cocos-first / OCR-fallback 頁面辨識 + 共用 `PageState` enum；`try_detect_main_page_fast` 僅由 `game_actions/stage_guard.py:45` 呼叫；`game_initialization.py:206` 使用的是 `PageState` / `detect_known_h5_page`，`utils/cocos_navigator.py:334` 則建立 `PageDetector` 呼叫 `detect()`；各路徑均受 per-device flag 控制 |
