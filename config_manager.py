        ...
        try:
            ...
            if "global" not in data:
                ...
                if "host_settings" not in data["global"]:
                    data["global"]["host_settings"] = DEFAULT_GLOBAL_CONFIG["host_settings"].copy()
                    changed = True
                else:
                    # 補全缺少的主機設定，避免共用設定檔被舊內容覆蓋後失效
                    for host_name, host_cfg in DEFAULT_GLOBAL_CONFIG["host_settings"].items():
                        if host_name not in data["global"]["host_settings"]:
                            data["global"]["host_settings"][host_name] = host_cfg.copy()
                            changed = True

                # 3. 補全 OCR 設定
                if "ocr" not in data["global"] or not isinstance(data["global"].get("ocr"), dict):
...
def get_ocr_config() -> Dict[str, Any]:
    ...
    merged["servers"] = [str(s).strip().rstrip('/') for s in merged["servers"] if str(s).strip()]
...
def update_ocr_config(new_settings: Dict[str, Any]):
    ...
    current["timeout_sec"] = max(1, _to_int(current.get("timeout_sec"), DEFAULT_OCR_CONFIG["timeout_sec"]))
    current["img_decode_retries"] = max(1, _to_int(current.get("img_decode_retries"), DEFAULT_OCR_CONFIG["img_decode_retries"]))
    current["ocr_empty_retries"] = max(0, _to_int(current.get("ocr_empty_retries"), DEFAULT_OCR_CONFIG["ocr_empty_retries"]))
    current["retry_delay_sec"] = max(0.0, _to_float(current.get("retry_delay_sec"), DEFAULT_OCR_CONFIG["retry_delay_sec"]))
