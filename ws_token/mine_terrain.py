...
# ── deterministic terrain (no learning) ──────────────────────────────────────
# The frontend never sends undug terrain over WS; it tiles configMine_template
# client-side. Reverse-engineered live (5558 + 5554 CDP, verified 54/54 undug
# cells across two accounts):
#
...
# rows = the 7-row viewport plus the look-ahead band). No CNN, no screenshot, no
# learning. Strictly more terrain than the CNN's 7-row viewport.
...
def _templates_by_id() -> Dict[int, Grid]:
    ...
    value holds — and NOT deduped (unlike ``_load_templates``).
    """
...
def terrain_at(depth: int, col: int, area_info: Dict[int, int]) -> Optional[int]:
    """Undug terrain {AIR=100, DIRT=201, STONE=202} for absolute (depth, col), or
    ``None`` when ``area_info`` does not cover that depth's area / col out of range.
    Pure config lookup — valid even for cells never dug or revealed."""
    c = int(col)
    if not (0 <= c < COLS):
        return None
    q = int(depth) + 6
