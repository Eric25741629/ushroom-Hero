...
def recall_one_cross(page: Any, slot_idx: int = 0) -> bool:
    """Recall one deployed cross-server car by its scrollHorse slot index.

    Flow (verified live 2026-05-20):
      1. Click `top/scrollHorse/view/content/<slot_idx>` cell
         → opens ParkingGainView
      2. Click `ParkingGainView/content/btnStart` (label: 領取)
         → opens TopView/MessageView/boxTips confirm dialog
      3. Click `MessageView/boxTips/dialog/content/buttons/btnEnsure` (確定)
         → server sends `cross_car_park_parking_stop_c2s` (cmd 0x3230)
         → scrollHorse cell removed, deployed count drops by 1

    Args:
        slot_idx: 0-based index in scrollHorse — pick a car to recall.

    Returns True iff the recall sequence completed and deployed count
    decreased (verified via post-snapshot).
    """
    if not _ensure_parking_main_open(page):
        return False
    # Close any stale popups that would block the scrollHorse click.
    # Important: dismiss BoxTip from prior recall, ParkingGainView, etc.
    try:
        page.evaluate(r"""() => {
          const um = window.uiMgr;
          for (const v of ['ParkingGainView','RedBagShowView','ParkingHorseManageView','ParkingHorseParkManageView','ParkingSpaceView']) {
            if (um.getView && um.getView(v)) um.close(v);
          }
          // BoxTip (in MessageView under TopView) — hide it directly
          const find = (root, parts) => { let n = root; for (const p of parts) { if (!n || !n.children) return null; n = n.children.find(c => (c.name||'')===p); if (!n) return null; } return n; };
          const boxTips = find(cc.director.getScene(), ['UIRoot','TopView','MessageView','boxTips']);
          if (boxTips && boxTips.active) boxTips.active = false;
        }""")
        ...
        pass
    # Snapshot before so we can verify
    before = get_deployed_cars(page)
    before_n = len(before)
    if not before:
        logger.info("[carpark_auto] no deployed cars to recall")
        return False
    if slot_idx >= len(before):
        logger.warning(f"[carpark_auto] recall slot {slot_idx} out of range (have {len(before)})")
        return False

    # Step 1: click scrollHorse cell at slot_idx
    coords = page.evaluate(f"""() => {{
      const find = (root, parts) => {{ let n = root; for (const p of parts) {{ if (!n || !n.children) return null; n = n.children.find(c => (c.name||'')===p); if (!n) return null; }} return n; }};
      const cell = find(cc.director.getScene(), ['UIRoot','NormalView','ParkingMainView','top','scrollHorse','view','content','{slot_idx}']);
      if (!cell) return null;
    ...
    if not coords:
        logger.warning(f"[carpark_auto] scrollHorse cell {slot_idx} not found")
        return False
    _click(page, coords["x"], coords["y"], action="recall.scrollHorse_cell",
           slot_idx=slot_idx)
    time.sleep(2.0)

    # Step 2: verify ParkingGainView opened, click btnStart (領取).
    # Sometimes the first click doesn't register (scrollHorse animation /
    # virtual list refresh). Retry once after a brief wait.
    gv_open = bool(page.evaluate(
        "(() => { const v = window.uiMgr.getView('ParkingGainView'); return !!(v && v.node && v.node.active); })()"
    ))
    if not gv_open:
        time.sleep(1.5)
        _click(page, coords["x"], coords["y"], action="recall.scrollHorse_cell_retry",
               slot_idx=slot_idx)
        time.sleep(2.5)
        gv_open = bool(page.evaluate(
            "(() => { const v = window.uiMgr.getView('ParkingGainView'); return !!(v && v.node && v.node.active); })()"
    ...
    if not gv_open:
        logger.warning("[carpark_auto] ParkingGainView didn't open after scrollHorse click (retried)")
        return False
    bs_coords = page.evaluate(r"""() => {
      const find = (root, parts) => { let n = root; for (const p of parts) { if (!n || !n.children) return null; n = n.children.find(c => (c.name||'')===p); if (!n) return null; } return n; };
      const b = find(cc.director.getScene(), ['UIRoot','NormalView','ParkingGainView','content','btnStart']);
      if (!b) return null;
      const wp = new cc.Vec3(); b.getWorldPosition(wp);
      const v = cc.view.getVisibleSize();
      const r = document.querySelector('canvas').getBoundingClientRect();
    ...
    if not bs_coords:
        logger.warning("[carpark_auto] btnStart not found in ParkingGainView")
        return False
    _click(page, bs_coords["x"], bs_coords["y"], action="recall.btnStart",
           slot_idx=slot_idx)
    time.sleep(1.8)

    # Step 3: click btnEnsure in confirm dialog
    en_coords = page.evaluate(r"""() => {
      const find = (root, parts) => { let n = root; for (const p of parts) { if (!n || !n.children) return null; n = n.children.find(c => (c.name||'')===p); if (!n) return null; } return n; };
      const b = find(cc.director.getScene(), ['UIRoot','TopView','MessageView','boxTips','dialog','content','buttons','btnEnsure']);
      if (!b || !b.activeInHierarchy) return null;
      const wp = new cc.Vec3(); b.getWorldPosition(wp);
      const v = cc.view.getVisibleSize();
      const r = document.querySelector('canvas').getBoundingClientRect();
      return {x: Math.round(r.left + wp.x*r.width/v.width), y: Math.round(r.top + (v.height-wp.y)*r.height/v.height)};
    ...
    if not en_coords:
        logger.warning("[carpark_auto] confirm dialog (btnEnsure) didn't appear")
        return False
    _click(page, en_coords["x"], en_coords["y"], action="recall.btnEnsure",
           slot_idx=slot_idx)
    time.sleep(3.5)

    # Verify deployed count dropped
    after = get_deployed_cars(page)
    success = len(after) < before_n
    logger.info(f"[carpark_auto] recall slot {slot_idx}: deployed {before_n} → {len(after)} "
                f"({'OK' if success else 'NO CHANGE'})")
    return success


def recall_n_cross(page: Any, n: int) -> int:
    """Recall up to N cross-server deployed cars. Returns the actual count
    recalled. Picks the OLDEST (highest slot_idx, longest timer) first by
    default — those have already capped on stack rewards (`at_limit`).

    Strategy: re-snapshot after each recall (slot indices shift as cells
    get removed). Prefer at_limit cars (max rewards already earned),
    then fall back to non-at_limit.
    """
    # Aggressively close any sub-view that would intercept clicks + force
    # ParkingMainView to home view (bottom/btnBack). The scrollHorse cell
    # click is finicky — needs a clean home-park view to register reliably.
    try:
        page.evaluate(r"""() => {
          const um = window.uiMgr;
          for (const v of ['ParkingGainView','RedBagShowView','ParkingHorseManageView','ParkingHorseParkManageView','ParkingSpaceView','ParkingRobView','GoodsGetView','ItemTipsView']) {
            if (um.getView && um.getView(v)) um.close(v);
          }
        }""")
        time.sleep(1.5)
    ...
    recalled = 0
    for _ in range(n):
        cars = get_deployed_cars(page)
        if not cars:
            break
        # Pick highest-value-to-recall: at_limit first (got full rewards), then
        # 100% HP, then by slot_idx descending
        at_limit_first = sorted(
            cars, key=lambda c: (not c.at_limit, -(c.hp_pct or 0), -c.slot_idx)
        )
        target = at_limit_first[0]
        # slot_idx in DeployedCar reflects the current scrollHorse position,
        # which IS the index to pass to recall_one_cross.
        if not recall_one_cross(page, slot_idx=target.slot_idx):
            logger.warning(f"[carpark_auto] recall failed at iteration {recalled}, stopping")
            break
        recalled += 1
        # Wait long enough for scrollHorse to re-index after the cell removal
        # animation completes. 0.5s wasn't enough — game needs ~2s before next click.
        time.sleep(2.0)
    return recalled
...
def park_one_normal(page: Any, **kwargs) -> Optional[str]:
    """Park one car as 一般 — onto MY OWN home park (`buildingRoot` 4 spots).

    "一般" cars are those deployed at MY OWN parking lot — visible in
    `scrollMiddleShow/.../buildingRoot` (4 spots). These are MY cars that
    sit at home generating rewards (compared to cross-server cars deployed
    at others' lots).

    Flow (validated 2026-05-20):
      1. Open ParkingMainView
      2. Click bottom/btnBack (回家) to return to OWN lot view if currently
         viewing someone else's lot
      3. buildingRoot becomes active (4 spots, often timers showing for
         OTHER players parked on me — but my own cars also live here)
      4. Find empty spot via nodeName.active → click it
      5. ParkingHorseParkManageView opens with picker
      6. Pick fresh-today car + 開始停車

    Returns parked car name on success, None if home park already full
    (4/4 deployed) or no fresh cars left.

    NOTE: 好友車位 tab (content/123) is for SNATCHING (ParkingRobView opens)
    not for normal deployment — confirmed live 2026-05-20.
    """
    if not _ensure_parking_main_open(page):
        return None

    # Close stale popups + close sub-views (ParkingSpaceView, RobView etc.)
    try:
        page.evaluate(r"""() => {
          const um = window.uiMgr;
          for (const v of ['GoodsGetView','ItemTipsView','RedBagShowView','ParkingGainView','ParkingHorseManageView','ParkingHorseParkManageView','ParkingSpaceView','ParkingRobView']) {
            if (um.getView && um.getView(v)) um.close(v);
          }
        }""")
        time.sleep(1.0)
    ...
    # Click bottom/btnBack to return to own park view (does nothing if already home)
    page.evaluate(r"""() => {
      const find=(r,p)=>{let n=r;for(const x of p){if(!n||!n.children)return null;n=n.children.find(c=>(c.name||'')===x);if(!n)return null;}return n;};
    ...
    # CRITICAL: verify buildingRoot (own home) is the active layer, not
    # buildingRoot1 (someone else's cross lot). Without this guard, parking
    # would deploy to a remote lot's empty spot — counted as cross, not home.
    active_root = page.evaluate(r"""() => {
      const find=(r,p)=>{let n=r;for(const x of p){if(!n||!n.children)return null;n=n.children.find(c=>(c.name||'')===x);if(!n)return null;}return n;};
      const content = find(cc.director.getScene(), ['UIRoot','NormalView','ParkingMainView','scrollMiddleShow','view','content']);
    ...
    if active_root != "buildingRoot":
        logger.warning(f"[carpark_auto] park_one_normal: not on home view (active={active_root}); aborting to avoid cross-park")
        return None

    # Now buildingRoot should be active. _click_empty_spot_in_current_lot
    # auto-detects which buildingRoot* is active.
    spot_idx = _click_empty_spot_in_current_lot(page)
    if spot_idx is None:
        logger.info("[carpark_auto] park_one_normal: home park full or not visible")
        return None

    # Picker should now be open
    name = _pick_zero_minute_car_and_park(page)
    if name:
        logger.info(f"[carpark_auto] park_one_normal: parked {name} at home/building{spot_idx}")
    return name
