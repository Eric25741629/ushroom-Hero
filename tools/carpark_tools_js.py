"""Legacy cocos-walk CDP payloads for 車位裝飾 (debug/tools only).

Dashboard 已改用純 WS (ws_token.carpark_decoration_ws)，不再需要瀏覽器。
以下 JS payload 僅供 tools/carpark_read_test.py 等獨立除錯工具使用。
"""
